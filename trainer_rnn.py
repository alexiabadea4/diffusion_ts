...
from diff_ts_rnn import GaussianDiffusionRNN

class Trainer:
    def __init__(
            self,
            net: GaussianDiffusionRNN,
        ...
        for epoch in range(self.epochs):
            ...
            with tqdm(train_iter, total=self.num_batches_per_epoch - 1) as it:
                for batch_no, data_entry in enumerate(it, start=1):
                    ...
                    quantiles = torch.rand(B, 1).to(signals.device)

                    t = torch.randint(0, self.net.num_timesteps, (B * T,), device=signals.device).long()
                    noise = torch.randn_like(signals.reshape(B*T, 1, -1))

                    x_noisy = self.net.q_sample(x_start=signals.reshape(B*T, 1, -1), t=t, noise=noise)
                    epsilon_theta = self.net.denoise_fn(x_noisy, t, class_labels, quantiles)
                    ...
                    losses = self.net.quantile_loss(epsilon_theta, noise, quantiles)
